...
module.exports = function finishHashing({ dir, completionFlags, buildEvents, hashingFileNameList, debug }) {
  ...
  if (debug) console.log(`${timestamp.stamp().red.bold}: finishHashing(): ${Object.keys(hashingFileNameList)}`);
  if (debug) console.log(`${timestamp.stamp().red.bold}: finishHashing(): completionFlags.ASSET_HASH.IMAGES :${completionFlags.ASSET_HASH.IMAGES}`);
  if (debug) console.log(`${timestamp.stamp().red.bold}: finishHashing(): completionFlags.ASSET_HASH.CSS    :${completionFlags.ASSET_HASH.CSS}`);
  if (debug) console.log(`${timestamp.stamp().red.bold}: finishHashing(): completionFlags.ASSET_HASH.JS     :${completionFlags.ASSET_HASH.JS}`);
